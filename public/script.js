...
let currentOpenTaskIndex = null; // Guarda qual tarefa está aberta no Modal
...
// --- LOGICA PRINCIPAL E RENDERIZAÇÃO DAS LINHAS ---
...
function renderTasks() {
    ...
    tasks.forEach((t, i) => {
        ...
        li.className = 'task-row'; // Visual de Tabela
        ...
        li.innerHTML = `
            ...
            <div><input type="date" class="date-edit" value="${t.deadline||''}" onchange="updateRowField(${i}, 'deadline', this.value)"></div>
            <div>
...
function addTask() {
    const t = document.getElementById('taskInput').value, req = document.getElementById('requesterSelect').value, dep = document.getElementById('deptSelect').value;
    if(!t) return;
    tasks.push({ text: t, requester: req||'Eu mesmo', dept: dep||'Administrativo', deadline: '', urgency: 'media', status: 'a-fazer', subtasks: [], comments: [] });
    document.getElementById('taskInput').value = ''; renderTasks(); saveTasks();
}
...
function closeModal() {
    ...
    renderTasks(); // Atualiza os contadores na tela principal
}
